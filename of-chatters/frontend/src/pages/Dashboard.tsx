...
function ExportMenu({ performanceData }: { performanceData: ChatterPerformance[] }) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen(v => !v)}
        className="px-4 py-2 rounded-lg font-medium transition-colors bg-white text-gray-700 hover:bg-gray-50 border"
      >
        Export
      </button>
      {open && (
        <div className="absolute right-0 mt-2 w-56 bg-white border rounded shadow-lg z-10 p-2">
          <button onClick={downloadTemplate} className="w-full text-left px-3 py-2 hover:bg-gray-50">Download Import Template (CSV)</button>
          <button onClick={exportCurrentView} className="w-full text-left px-3 py-2 hover:bg-gray-50">Export current view (CSV)</button>
        </div>
      )}
    </div>
  );
