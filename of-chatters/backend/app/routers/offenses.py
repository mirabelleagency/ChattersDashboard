...
def create_offense(payload: schemas.OffenseCreate, request: Request, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def update_offense(offense_id: int, payload: schemas.OffenseUpdate, request: Request, db: Session = Depends(get_db), user=Depends(get_current_user)):
...
def delete_offense(offense_id: int, request: Request, soft: bool = True, db: Session = Depends(get_db), user=Depends(get_current_user)):
    off = db.get(models.Offense, offense_id)
    if not off:
        raise HTTPException(status_code=404, detail="Offense not found")
    before = schemas.OffenseOut.model_validate(off).model_dump()
